...
export const cartItems = p.pgTable(
    'cart_items',
    {
        ...
        basePrice: p.numeric('base_price', { precision: 12, scale: 2 }).$type<number>().notNull(),
        totalPrice: p.numeric('total_price', { precision: 12, scale: 2 }).$type<number>().notNull(),
        ...
        offerId: p.uuid('offer_id').references(() => offers.id, { onDelete: 'set null' }),
        shopId: p.uuid('shop_id').references(() => shops.id, { onDelete: 'set null' }),
    },
    (table) => [p.index().on(table.offerTitle), p.index().on(table.quantity)],
);
