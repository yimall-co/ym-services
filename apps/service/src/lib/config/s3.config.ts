...
export default registerAs('s3', async () => {
    ...
    const s3Region = await lockerRead.get('S3_REGION');
    const s3Endpoint = await lockerRead.get('S3_ENDPOINT');
    const s3AccessKey = await lockerRead.get('S3_ACCESS_KEY');
    const s3SecretKey = await lockerRead.get('S3_SECRET_KEY');
    const s3BucketName = await lockerRead.get('S3_BUCKET_NAME');
